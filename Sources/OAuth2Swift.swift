...
open class OAuth2Swift: OAuthSwift {
    ...
    @discardableResult
    open func checkToken(checkURL:String, checkURLParameter:String, token: String, parameters: OAuthSwift.Parameters? = nil, headers: OAuthSwift.Headers? = nil, completionHandler completion: @escaping TokenCompletionHandler) -> OAuthSwiftRequestHandle? {
     
        var parameters = parameters ?? OAuthSwift.Parameters()
        parameters[checkURLParameter] = token
        
        let localCompleteHandler: OAuthSwiftHTTPRequest.CompletionHandler = { result in
            switch result {
            case .success(let response):
                let responseJSON: Any? = try? response.jsonObject(options: .mutableContainers)
                let responseParameters: OAuthSwift.Parameters
                ...
                completion(.success((tempCred, response, responseParameters)))
                break
        ...
        return self.client.request(checkURL, method: .GET, parameters: parameters, headers: headers, checkTokenExpiration: false, completionHandler: localCompleteHandler)
    }
}
